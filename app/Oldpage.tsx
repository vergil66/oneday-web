...
type Entry = {
  ...
  inheritedPlace?: boolean;
};

type EntryMap = Record<string, Entry>;
type ViewMode = "today" | "review" | "settings";
...
function getPreviousEntry(entries: EntryMap, currentDate: string): Entry | null {
...
function makeEmptyEntry(date: string, previous?: Entry | null): Entry {
  return {
    id: date,
    date,
    title: "",
    ...
    inheritedLocation: Boolean(previous?.location),
    inheritedPlace: Boolean(previous?.place),
  };
}
...
export default function OnedayWebPrototype() {
  ...
  const previousEntry = getPreviousEntry(entries, currentDate);
  
  const currentEntry: Entry =
    entries[currentDate] ?? makeEmptyEntry(currentDate, previousEntry);
  ...
  return (
    <div className="min-h-screen bg-stone-50 text-stone-900">
      <div className="mx-auto max-w-7xl px-4 py-6 md:px-6 md:py-8">
        ...
        >
          <aside className="rounded-3xl border border-stone-200 bg-white p-4 shadow-sm lg:sticky lg:top-6 lg:h-fit">
            <div className="mb-4">
              <div className="text-xs uppercase tracking-[0.2em] text-stone-500">
                Vergil’s Coffee Table
              </div>
              <h1 className="mt-2 text-2xl font-semibold">Oneday</h1>
              <p className="mt-2 text-sm leading-6 text-stone-600">
                Begin with the day itself: where you are, what is true, and what must be kept, grown, and tended.
              </p>
            </div>
            ...
            <div className="grid gap-2">
              ...
              >
                <CalendarDays size={16} /> Export & Import
              </button>
            </div>
          ...
          <main>
            {view === "today" && (
              <section className="rounded-3xl border border-stone-200 bg-white p-4 shadow-sm md:p-6">
                ...
                <div className="mt-5 grid gap-4">
                  ...
                  <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    <FieldCard
                      ...
                      inherited={currentEntry.inheritedLocation}
                      onChange={(value) =>
                        patchCurrentEntry({
                          location: value,
                    ...
                    <FieldCard
                      ...
                      inherited={currentEntry.inheritedPlace}
                      onChange={(value) =>
                        patchCurrentEntry({
                          place: value,
                          inheritedPlace: false,
                        })
                      }
                    />
                    <FieldCard
  ...
  return (
    <div className="rounded-2xl border border-stone-200 bg-stone-50/70 p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          onClick={onAdd}
          className="inline-flex items-center gap-2 rounded-2xl bg-white px-3 py-2 text-sm text-stone-700 shadow-sm ring-1 ring-stone-200 hover:bg-stone-100"
        >
          <Plus size={16} /> Add
        </button>
      </div>
      ...
      <div className="mt-4 grid gap-3">
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <div
              key={item.id}
              className="grid grid-cols-[auto_1fr_auto] items-start gap-3 rounded-2xl border border-stone-200 bg-white p-3"
            >
              <input
                type="checkbox"
                checked={item.done}
              ...
              <textarea
                ...
                rows={2}
                className="w-full resize-none rounded-xl border border-stone-200 px-3 py-2 text-sm leading-6 outline-none transition placeholder:text-stone-400 focus:border-stone-500"
              />
              <button
